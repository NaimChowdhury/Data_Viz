...
ggplot(data= lang_table, aes(x=reorder(othlang,props),y=props, fill=othlang)) + 
  ...
  ggtitle(label="Can the respondent speak language other than English/Spanish?", subtitle="Based on 2018 GSS") +
  annotate("text", x = c('Yes', 'No'), y = c(30.8, 69.2) -4, label = c("30.8%", "69.2%"), color = "white", size = 2)
```


```{r}
pie <- ggplot(data = lang_table, aes(y = props, fill = othlang)) + 
