...
fit2 <- glm(sat ~ width + color1 + width*color,family = binomial(link = logit), crab)
...
plot.probs <- ggplot(data = plotdata, aes(x = balance, y = prob, color = student)) +
  ...
  scale_color_manual(values = mycol, name = "Student:") +
  scale_fill_manual(values = mycol, name = "Student:") +
  theme_bw() +
